...
import {
  ...
  useSaveGroup,
  useDeleteGroup,
  useAddAccountToRegion,
  useRemoveAccountFromRegion,
  useUpdateRegionGroup,
...
import {
  ...
  ProvinceDialog,
  GroupDialog,
} from "@/components/features/regions/region-form-dialog";
...
export default function RegionManagement() {
  ...
  const provinces = useMemo(() => data?.provinces ?? [], [data?.provinces]);
  ...
  const groupRegions = useMemo(
    () =>
      groupDetailDialog.data
        ? regions.filter((r) => r.group_id === groupDetailDialog.data!.id)
        : [],
  ...
  async function handleDeleteRegion(region: Region) {
    const ok = await confirm({
      title: "Delete region?",
      description: `"${region.name}" and all account assignments will be removed.`,
      variant: "destructive",
      confirmLabel: "Delete",
    });
  ...
  async function handleDeleteProvince(p: Province) {
    const ok = await confirm({
      ...
      description: `"${p.name}" will be permanently deleted.`,
      variant: "destructive",
      confirmLabel: "Delete",
    });
  ...
  async function handleDeleteGroup(g: Group) {
    const ok = await confirm({
      title: "Delete group?",
      description: `"${g.name}" will be permanently deleted.`,
      variant: "destructive",
      confirmLabel: "Delete",
    });
  ...
  async function handleRemoveRegionFromGroup(region: Region) {
    const ok = await confirm({
      ...
      description: `"${region.name}" will be unassigned from this group.`,
      variant: "destructive",
      confirmLabel: "Remove",
    });
  ...
  const addButtons: Record<ActivePanel, React.ReactNode> = {
    regions: (
      <Button size="sm" onClick={() => regionFormDialog.open()}>
        <Plus className="size-4" />
        Add Region
      </Button>
    ),
    provinces: (
      <Button size="sm" onClick={() => provinceFormDialog.open()}>
        <Plus className="size-4" />
        Add Province
      </Button>
    ),
    groups: (
      <Button size="sm" onClick={() => groupFormDialog.open()}>
        <Plus className="size-4" />
        Add Group
      </Button>
    ),
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Manage regions, provinces, and content groups
          </p>
        </div>
      ...
      <Dialog open={accountsDialog.isOpen} onOpenChange={accountsDialog.onOpenChange}>
        ...
        >
          <div className="px-6 pt-6 pb-4 border-b border-border shrink-0">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              {accountsDialog.data?.province_name}
              {accountsDialog.data?.group_name ? ` · ${accountsDialog.data.group_name}` : ""} ·{" "}
              <strong className="text-foreground font-medium">
                {accountsDialog.data?.account_count ?? 0} accounts
              </strong>
            </p>
            <div className="mt-3">
              <Button
                size="sm"
                variant="outline"
                onClick={() => accountSelectorDialog.open()}
                ...
                <TableBody>
                  {regionAccounts.map((ra) => (
                    <TableRow key={ra.id}>
                      ...
                      <TableCell>
                        <Badge variant={ra.is_active ? "default" : "outline"}>
                      ...
                      <TableCell className="text-right">
                        <Button
                          ...
                          size="xs"
                          onClick={() =>
                            handleRemoveAccount(accountsDialog.data!.id, ra.account_id, ra.username)
                          }
        ...
        >
          <div className="px-6 pt-6 pb-4 border-b border-border shrink-0">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              ...
              {groupRegions.reduce((s, r) => s + r.account_count, 0)} total accounts
            </p>
          </div>
          <div className="flex-1 overflow-auto min-h-0">
                      ...
                      <TableCell className="text-right">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <Button
                            variant="ghost"
                            size="xs"
                            onClick={() => handleRemoveRegionFromGroup(region)}
                            className="text-muted-foreground hover:text-destructive hover:bg-destructive/10"
                          >
                            Remove
                          </Button>
