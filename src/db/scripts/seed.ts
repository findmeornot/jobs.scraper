...
async function seedData() {
  try {
    ...
    // 1. Master Category
    logger.info("Seeding master_category...");
    ...
    logger.info(`Finished master_category (${categories.length})`);

    // 2. Master Province
    logger.info("Seeding master_province...");
    ...
    logger.info(`Finished master_province (${provinces.length})`);

    // 3. Master Group
    logger.info("Seeding master_group...");
    ...
    logger.info(`Finished master_group (${groups.length})`);

    // 4. Master Region
    logger.info("Seeding master_region...");
    ...
    logger.info(`Finished master_region (${regions.length})`);

    // 5. Instagram Account
    logger.info("Seeding instagram_account...");
    ...
    logger.info(`Finished instagram_account (${accounts.length})`);

    // 6. Region Account
    logger.info("Seeding region_account...");
    ...
    logger.info(`Finished region_account (${regionAccounts.length})`);

    logger.info("Database seeding completed successfully!");
    process.exit(0);
  } catch (error) {
    logger.error({ error }, "Error during seeding");
    process.exit(1);
  }
}
